...
export class AppComponent {
  ...
  constructor(
    ...
    // Get month Summary
    const summary$ = fireService.getById<Summary>(
      ENTITY.SUMMARY,
      this.getSummaryId()
    );
  ...
  getSummaryId(): string {
    ...
    if (today.getDate() >= 25) {
      ...
      id = `${today.getFullYear()}${today.getMonth()}-${today.getFullYear()}${
        today.getMonth() + 1
      }`;
    }
    return id;
  ...
  // Get All
  getAll() {
    collectionData(collection(this.firestore, ENTITY.EXPENSE));
  }
  ...
  add() {
    ...
    addDoc(collectionRef, {}).then(() => console.log('added'));
  }

  // Update
  update(id: string) {
    const customerRef = doc(this.firestore, ENTITY.EXPENSE, id);
    updateDoc(customerRef, {}).then(() => console.log('updated'));
  }
}
